...
int main(int argc, char **argv)
{
	...
	if (!strcmp(argv[1], "start")) {
		...
		printf("cpu,seq,func,eth_protocol,ip_protocol,ip_saddr,ip_daddr,"
			   "tp_sport,tp_dport,time,skb_id,sock_id,pkglen,cnt,rec_qlen,bklg_qlen,ip_frag,tcp_seq,ip_id\n");
		for (i = 0; i < NSL_MAX_CPU; i++) {
			...
			for (j = 0; j <= cur; j++) {
				...
				printf("%d,%d,%d,%d,%d,%s,%s,"
				       "%d,%d,%llu,%llu,%llu,%llu,%u,%u,%u,%u,%u,%u\n",
				       ...
				       nsl_table[idx].ip_frag_off,
				       nsl_table[idx].tcp_seq,
				       nsl_table[idx].ip_id
				       );
			}
		}
